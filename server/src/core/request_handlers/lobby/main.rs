...
use commands::{
    become_role_cmd, check_error, clear_cmd, close_lobby_cmd, connect_cmd, create_lobby_cmd,
    disconnect_cmd, get_lobbies_cmd, join_lobby_cmd, leave_lobby_cmd, make_host_cmd, ping_cmd, change_name_cmd, get_lobby_state,
};
use events::EventLoop;
use types::{GameState, GameStateShared, UserType, LobbyVec, LobbyShort};
    ...
    loop {
        ...
        if buf == "ping" {
            ...
            let new_name = buf
                ...
                .nth(1)
                .unwrap();
            let new_name = String::from(&buf[new_name..]);

            match change_name_cmd(&state.id, new_name.clone()) {
